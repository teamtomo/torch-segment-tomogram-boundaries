...
def generate_volume_mask(planes, reference_mrc_path, output_mrc_path):
    ...
    with mrcfile.open(reference_mrc_path, permissive=True) as mrc:
        ...
        header_copy = mrc.header.copy()  # Copy the full header
    ...
    with mrcfile.new(output_mrc_path, overwrite=True) as mrc_out:
        mrc_out.set_data(mask_volume)
        # Use the copied header to preserve all metadata like voxel size
        mrc_out.header = header_copy
...
def main(args):
    ...
    for imod_path in imod_files:
        ...
        try:
            ...
            generate_volume_mask(planes, ref_tomo_path, output_mask_path)
